...
一句话总结：把投标审查报告固化为可长期使用的 12 页彩色 Excel 模板，补齐空页数据源与项目信息/预测得分，
提高大文件链路上传能力；同步完成品牌升级和前端/后端 RBAC 权限闭环。

### ✨ 新增（Added）

- **投标审查固定 Excel 模板** (`0b1c4ee`)
  - 输出固定为 12 个彩色标签页，顺序为：项目信息、审查概要与风险、评分项分析与预测、废标项核查、分项报价、交付时间对比、时间节点、废标项核对(BidMaster)、▲参数核对、证明材料清单、时间节点核对、合同条款要点。
  - 高/中/低风险行使用统一底色；缺失页保留标准“待补充”占位，保证每次下载的报告结构一致。
  - 项目信息表补入招标项目、投标单位、招标单位等基础字段；评分项必须输出 `predicted_score` 和预测理由。
  - Excel 组装逻辑拆分到 `tender_review_template.py`，后续模板调整不再影响模型审查流程。

- **前端 RBAC 权限路由** (`9eb7ff0`)
  - 登录后保存权限码，前端按模块控制招标解读、投标生成、投标检查、文档排版、资讯中心和系统设置入口。
  - 无权限用户访问受保护路由时重定向到工作台，避免界面暴露不可用功能。

### 🔧 调整（Changed）

- **补全投标审查空页数据源** (`a490c39`)
  - 项目信息、审查概要与风险、废标项核查、分项报价、交付时间对比和时间节点等页面从审查结果中补充数据，不再输出无意义空白页。
  - 保留模板统一结构和占位口径，便于不同项目的审查结果横向对比。

- **品牌与界面统一** (`0f5a29e` `f96e953` `c0f60f0`)
  - 系统名统一为「智多星标书辅助系统」，英文名保留为 Resourceful Star；标题、侧边栏、版本说明、Docker 描述、健康接口和技能元数据同步更新。
  - 更换新 logo 与 favicon，登录页仅显示中文系统名，移除界面英文副标题。
  - 登录页不再预填或展示默认管理员账号密码；设置页管理提示改为「系统内置管理员」，避免凭据线索暴露。

- **RBAC 接口保护** (`9eb7ff0`)
  - `/auth/login` 和 `/auth/me` 返回用户权限码，登录前端据此渲染菜单和路由。
  - 招标解读、投标生成、投标检查、文档排版四个业务路由统一挂载登录鉴权依赖，未认证请求不能直接调用后台能力。

### 🐞 修复（Fixed）

- **大文件上传 413** (`bf28699` `a2f98e1`)
  - 提高 Nginx 和 API 层投标审查上传上限，两个入口限制保持一致，避免大标书在代理层或应用层被不同规则拒绝。

### 🧪 验证（Verification）

- 后端核心路由通过 Python `py_compile` 编译检查。
- 前端通过 TypeScript 编译和 Vite 生产构建。
- Web 镜像已在 `192.168.50.31` 重建部署；线上 JS 产物确认不再包含默认账号提示、`admin123` 或界面英文副标题。
- 线上容器 `bidmaster-web-1` healthy，`/api/health` 返回 `db_ready: true`。
...
[0.3.1]: https://github.com/andyke668/BidMaster-Pro/compare/v0.3.0...v0.3.1
